...
with open('case_log.txt', mode='r', encoding='utf-8') as f:
    for line in f.readlines():
        ret = re.search(r'(.*)\s:\sACTION\s(\w+)\s\[started\].*', line)
        ret1 = re.search(r'(.*)\s:\sACTION\s(\w+)\s\[finished\].*duration\=(\d+)\.(\d){3}sec', line)
    if ret:
        action_starttime = ret.group(1)
        x = ((action_starttime.split(" "))[1]).split(":")
        x[2] = x[2].replace(',', '')
    if ret1:
        action_name = ret1.group(2)
        action_endtime = ret1.group(1)
        y = ((action_endtime.split(" "))[1]).split(":")
        y[2] = y[2].replace(',', '')
        action_time = ((float(y[0]) - float(x[0])) * 3600 + (float(y[1]) - float(x[1])) * 60 + (float(y[2]) - float(x[2]))) / 1000
    ...
    for key in sorted(action_dict):
        ...
        print(
            '#------------------------------------- The next will be write data to excel ----------------------------------------------#')
...
d_style3 = {'font_color': 'black', 'background_color': 'yellow', 'h_align_mode': 'HORZ_CENTER',
            'v_align_mode': 'VERT_CENTER'}
...
with open('case_log.txt', mode='r', encoding='utf-8') as f:
    for line in f.readlines():
        ret = re.search(r'ACTION\s(\w+)'
                        r'\s\[finished\].*duration\=((\d+)\.(\d){3})sec', line)
        if ret:
